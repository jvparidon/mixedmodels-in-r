...
This took just under three minutes for a very simple model. Doing the same in `Julia` will be much faster, but it works a little differently:
...
The function is marginally slower than the previous bootstrap we did in `Julia`, because we include the fitting of the original model in the function block (and therefore it's captured by the timing function, as well) but this is still more than an order of magnitude faster than computing CIs in `R`!
